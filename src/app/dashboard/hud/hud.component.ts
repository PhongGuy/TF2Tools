...
export class HudComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.library.length === 0) {
      ...
      if (this.electron.fs.existsSync(oldPath)) {
        ...
        this.app.log.next(
          `Trying to move files from "${oldPath}" => "${this.localHuds}"`
        );
        this.electron.fs
          .move(oldPath, this.localHuds, { overwrite: true })
          .then(() => {
            oldPath = this.electron.appData("TF2Tools\\hitsounds");
            if (this.electron.fs.existsSync(oldPath)) {
              this.app.log.next(
                `Trying to move your hitsounds from "${oldPath}" => "${this.app.settings.libraryPath}\\hitsounds"`
              );
              this.electron.fs
                .move(oldPath, `${this.app.settings.libraryPath}\\hitsounds`, {
                  overwrite: true,
                })
                .then(() => {
  ...
  update() {
    ...
    this.electron.fs.readdirSync(this.localHuds).forEach((customFiles) => {
      ...
      if (this.electron.fs.existsSync(info)) {
        const data = this.electron.fs
          .readFileSync(info, { encoding: "utf8", flag: "r" })
          .split('"');
        const h = {
          name: data[1],
          folderName: customFiles,
          version: data[5],
          path: this.localHuds + "\\" + customFiles,
        };
        this.library.push(h);
      }
    });
    ...
    this.notInLibrary = [];
    this.electron.fs
      .readdirSync(this.app.settings.customPath)
      .forEach((customFiles) => {
        const info =
          this.app.settings.customPath + "/" + customFiles + "/info.vdf";
        if (this.electron.fs.existsSync(info)) {
          this.hudsInstalled++;
          ...
          const h = {
            ...
            folderName: customFiles,
            version: data[5],
            path: this.app.settings.customPath + "\\" + customFiles,
          };
          const l = {
            name: data[1],
            folderName: customFiles,
  ...
  add(_hud: Hud) {
    this.app.log.next(
      `Installing hud: *COPY* "${_hud.path}" => "${this.app.settings.customPath}\\${_hud.folderName}"`
    );
    this.electron.fs
      .copy(_hud.path, `${this.app.settings.customPath}\\${_hud.folderName}`)
      .then(() => {
  ...
  uninstall(_hud: Hud) {
    if (this.electron.fs.existsSync(_hud.path)) {
      if (this.app.settings.moveOrCopy) {
        this.app.log.next(
          `Uninstalling hud: *MOVE* "${_hud.path}" => "${this.localHuds}\\${_hud.folderName}"`
        );
        this.electron.fs
          .move(_hud.path, `${this.localHuds}\\${_hud.folderName}`, {
            overwrite: true,
          })
          .then(() => {
        ...
        this.electron.fs.remove(_hud.path).then(() => {
          this.snack.show(`${_hud.folderName} was uninstalled`);
          this.update();
        });
      }
  ...
  upload(event: Event) {
    ...
    if (hudsFound.length > 0) {
          ...
          this.electron.fs
            ...
            .then(() => {
              ...
              this.snack.show(
                ...
                null,
                hudsFound.length > 2 ? 1000 : 3000
              );
              const data = this.electron.fs
                .readFileSync(hud.path, { encoding: "utf8", flag: "r" })
                .split('"');
              const l = {
                name: data[1],
                folderName: name,
                version: data[5],
                path: this.localHuds + "\\" + name,
              };
              this.library.push(l);
              this.myControl.setValue("");
            })
      ...
      const s =
        "We could not find any hud here. Are you sure you selected the right folder and it has the info.vdf file inside.";
  ...
  rename(_hud: Hud): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (typeof result === "string") {
        ...
        const name = path.pop();
        this.app.log.next(
          `Renaming hud: *RENAME* "${_hud.folderName}" => "${name}"`
        );
        this.electron.fs
          .rename(_hud.path, `${path.join("\\")}\\${result}`)
          .then(() => {
