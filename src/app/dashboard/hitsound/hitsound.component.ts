...
export class HitsoundComponent implements OnInit {
  ...
  async update() {
    ...
    this.electron.fs.readdirSync(this.localHitsounds).forEach((file) => {
      ...
      const h = {
        name: name.toString(),
        path: `${this.localHitsounds}\\${name}.wav`,
      };
      this.library.push(h);
    });
    ...
    for (const sound of files) {
      if (sound.name.includes(".wav")) {
        ...
        if (name === "hitsound" || name === "killsound" || exist) {
          ...
          await firstValueFrom(dialogRef.afterClosed()).then((newName) => {
            if (typeof newName === "string") {
              this.electron.fs
                .copy(sound.path, `${this.localHitsounds}\\${newName}.wav`)
                .then(() => {
    ...
    if (this.electron.fs.existsSync(_hitsound.path)) {
      ...
      dialogRef.afterClosed().subscribe((r) => {
        if (typeof r === "string") {
          ...
          path.pop();
          this.app.log.next(
            `Renaming hitsound: *RENAME* "${_hitsound.path}" => "${path.join(
              "\\"
            )}\\${r}.wav"`
          );
          this.electron.fs
            .rename(_hitsound.path, `${path.join("\\")}\\${r}.wav`)
            .then(() => {
  ...
  remove(_hitsound: Hitsound) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.electron.fs
          .remove(_hitsound.path)
          .then(() => {
            this.snack.show(`${_hitsound.name} was removed`);
            if (
              _hitsound.name === "hitsound" ||
              _hitsound.name === "killsound"
            ) {
              this.app.update("hitsounds");
            } else {
              this.update();
  ...
  play(_hitsound: Hitsound): void {
    if (_hitsound.name === "hitsound") {
      ...
      _hitsound.path = `${this.app.appTemp}\\${this.app.generateRandomString(
        8
      )}.wav`;
  ...
  installHitsound(_hitsound: Hitsound): void {
    ...
    if (path !== undefined) {
      this.app.log.next(
        `Installing hitsound: *COPY* "${_hitsound.path}" => "${path}"`
      );
      this.electron.fs
        .copy(_hitsound.path, path, { overwrite: true })
        .then(() => {
      ...
      this.electron.fs
        ...
        .then(() => {
          this.app.log.next(
            `Installing hitsound: *COPY* "${_hitsound.path}" => "${defaultPath}\\hitsound.wav"`
          );
          this.electron.fs
            .copy(_hitsound.path, `${defaultPath}\\hitsound.wav`)
            .then(() => {
              this.snack.show(`Hitsound ${_hitsound.name} was installed`);
              this.app.update("hitsounds");
            })
  ...
  installKillSound(_killsound: Hitsound): void {
    ...
    if (path !== undefined) {
      this.app.log.next(
        `Installing killsound: *COPY* "${_killsound.path}" => "${path}"`
      );
      this.electron.fs
        .copy(_killsound.path, path, { overwrite: true })
        .then(() => {
      ...
      this.electron.fs
        ...
        .then(() => {
          this.app.log.next(
            `Installing killsound: *COPY* "${_killsound.path}" => "${defaultPath}\\killsound.wav"`
          );
          this.electron.fs
            .copy(_killsound.path, `${defaultPath}\\killsound.wav`)
            .then(() => {
              this.snack.show(`Killsound ${_killsound.name} was installed`);
              this.app.update("hitsounds");
            })
